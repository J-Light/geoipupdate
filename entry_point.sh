...
! (: "${GEOIP_CONFIG_FILE?}") 2>/dev/null && {
    GEOIP_CONFIG_FILE="/etc/geoipupdate/GeoIP.conf"
    [[ ! -z $GEOIP_USER_ID ]] && {
    ...
    echo "ProductIds ${GEOIP_PRODUCT_IDS:-"GeoLite2-Country"}" >> $GEOIP_CONFIG_FILE
    echo "DatabaseDirectory ${GEOIP_DIRECTORY:-"/usr/local/geoipupdate"}" >> $GEOIP_CONFIG_FILE
}
